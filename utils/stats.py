...
def get_longest_simpoints(weights):
    """DEPRECATED"""
    warnings.warn(
        'get_longest_simpoints is deprecated, use -w in eval script instead.')
    idx = (weights.groupby('trace')[
           'weight'].transform(max) == weights['weight'])
...
def _process_prefetcher(stats, df, weights, tr, pf, plt):
    ...
    stats['pythia_level_threshold'] = np.append(
        stats['pythia_level_threshold'], plt)
    ...
    for metric in utils.metrics:
        target = data[metric].item() if len(data) <= 1 else utils.mean(
            data[metric], metric, weights=weights)
...
def get_weighted_statistics(df, weights):
    """DEPRECATED"""
    warnings.warn(
        'get_weighted_statistics is deprecated, use -w in eval script instead.')
    stats = {
        'trace': np.array([]),
        'all_pref': [],
