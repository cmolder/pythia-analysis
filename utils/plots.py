...
def plot_metric(table: pd.DataFrame,
    ...
    for k, x in Xs.items():
        if k in ys_secondary:
            ax.bar(x, ys_secondary[k], color='lightgray', width=1.0, edgecolor='gray', linewidth=0.3, zorder=0.99)
    ...
    # Y-axis
    ytick_dist = metric_tick_distances(metric) if ytick_distance is None else ytick_distance
    ...
    if add_ylabel:
        ylabel = metric.replace('_', ' ') if ylabel == '' else ylabel
